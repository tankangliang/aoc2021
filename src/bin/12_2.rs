        ...
        for neighbour in hs {
            if neighbour == "end" {
                ...
                *count += 1;
                ...
                match (visited.get(neighbour), is_big, twice) {
                    (_, true, _) => visit(neighbour, count, adj_list, visited, twice),
                    (None, _, _) => visit(neighbour, count, adj_list, visited, twice),
                    (Some(visit_count), false, true) if *visit_count == 0 => visit(neighbour, count, adj_list, visited, twice),
                    (Some(visit_count), false, false) if *visit_count == 0 => visit(neighbour, count, adj_list, visited, twice),
                    (Some(visit_count), false, false) if *visit_count == 1 => visit(neighbour, count, adj_list, visited, true),
                    _ => {}
                }
            }
...
fn main() {
    ...
    visit("start", &mut count, &adj_list, &mut visited, false);
    println!("{}", count);
}
